...
decl : type  { currentType = (TS_entry)$1; } id ';'
	 ...
	 | FUNC VOID { currentType = Tp_VOID; } id '(' { escopoLocal = new TabSimb(); } lparam ')' 
		'{' { ts = escopoLocal; } corpoF { ts = aux; } '}' { TS_entry nodo = ts.pesquisa($4);
...
pexp : exp { 
	...
	if ( pilhaEscopo.peek().get(pilhaPosicao.peek()).getTipo() != (TS_entry)$1 ||  pilhaEscopo.peek().get(pilhaPosicao.peek()).getClasse() != ClasseID.NomeParam ) {
	   System.out.println("sif");
	   System.out.println(pilhaEscopo.peek().get(pilhaPosicao.peek()).getTipo());
	   System.out.println(pilhaEscopo.peek().get(pilhaPosicao.peek()).getClasse());
